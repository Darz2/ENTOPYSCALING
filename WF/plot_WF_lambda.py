...
with plt.style.context([ 'ieee']):
    ...
    for i, lambda_val in enumerate(Lambda):
        ...
        V_WF = wf_potential(r, rc, alpha, lambda_val)
        if np.any(np.abs(V_WF) > 1e17):  # Set a reasonable threshold
    ...
    cbar.set_label(r"$\lambda$", labelpad=-40, fontsize=label_fontsize)  # Label for color bar
